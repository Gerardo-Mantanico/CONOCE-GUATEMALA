...
-- **********************************************************************
-- Schema: artesania
-- Módulo de Arte y Artesanía del Altiplano Occidental.
-- **********************************************************************
-- **********************************************************************
...
CREATE TABLE IF NOT EXISTS artesania.tipo_obra_arte (
    ...
    nombre            VARCHAR(120) NOT NULL UNIQUE,
    descripcion       TEXT,
    categoria_general VARCHAR(100) NOT NULL,
    carga_id          INT REFERENCES meta.carga(id)
);

COMMENT ON TABLE artesania.tipo_obra_arte IS 'Catálogo de tipos de obras o especialidades de arte y artesanía.';
COMMENT ON COLUMN artesania.tipo_obra_arte.id_tipo_obra IS 'Identificador único del tipo de obra.';
COMMENT ON COLUMN artesania.tipo_obra_arte.codigo IS 'Código estable de identificación del tipo de obra.';
COMMENT ON COLUMN artesania.tipo_obra_arte.nombre IS 'Nombre del tipo de obra (ej. Pintura Paisajista, Tejido de Algodón).';
COMMENT ON COLUMN artesania.tipo_obra_arte.descripcion IS 'Descripción del tipo de obra y sus características generales.';
COMMENT ON COLUMN artesania.tipo_obra_arte.categoria_general IS 'Categoría mayor de agrupación (ej. TEXTIL, PINTURA, MADERA).';
...
COMMENT ON TABLE artesania.tecnica_artesanal IS 'Catálogo de técnicas tradicionales y contemporáneas aplicadas en la elaboración de piezas.';
COMMENT ON COLUMN artesania.tecnica_artesanal.id_tecnica IS 'Identificador único de la técnica.';
COMMENT ON COLUMN artesania.tecnica_artesanal.codigo IS 'Código estable de identificación de la técnica.';
COMMENT ON COLUMN artesania.tecnica_artesanal.nombre IS 'Nombre oficial de la técnica (ej. Telar de Cintura, Brocado, Dorado con Pan de Oro).';
COMMENT ON COLUMN artesania.tecnica_artesanal.origen_historico IS 'Información histórica y comunitaria sobre el origen de la técnica.';
COMMENT ON COLUMN artesania.tecnica_artesanal.descripcion IS 'Explicación detallada del proceso físico y conceptual de la técnica.';
...
COMMENT ON TABLE artesania.taller_colectivo IS 'Talleres familiares, cooperativas y colectivos de artesanos organizados.';
COMMENT ON COLUMN artesania.taller_colectivo.id_taller IS 'Identificador único del taller.';
COMMENT ON COLUMN artesania.taller_colectivo.codigo IS 'Código estable del taller o cooperativa.';
COMMENT ON COLUMN artesania.taller_colectivo.nombre IS 'Nombre comercial u oficial del taller o colectivo.';
COMMENT ON COLUMN artesania.taller_colectivo.representante IS 'Nombre del director, maestro artesano o representante legal.';
COMMENT ON COLUMN artesania.taller_colectivo.departamento_id IS 'Departamento oficial verificado del taller.';
COMMENT ON COLUMN artesania.taller_colectivo.municipio_id IS 'Municipio oficial verificado del taller.';
COMMENT ON COLUMN artesania.taller_colectivo.direccion IS 'Dirección exacta o referencia de ubicación del taller.';
COMMENT ON COLUMN artesania.taller_colectivo.anio_fundacion IS 'Año de fundación del taller o colectivo.';
COMMENT ON COLUMN artesania.taller_colectivo.cantidad_miembros IS 'Cantidad de artesanos activos en la organización.';
COMMENT ON COLUMN artesania.taller_colectivo.contacto IS 'Datos de contacto (teléfono, correo o red social).';
...
COMMENT ON COLUMN artesania.produccion_artesanal_municipal.volumen_estimado_unidades IS 'Volumen total estimado de unidades producidas al año.';
COMMENT ON COLUMN artesania.produccion_artesanal_municipal.valor_estimado_mercado_q IS 'Valor total estimado de mercado de la producción en Quetzales.';
COMMENT ON COLUMN artesania.produccion_artesanal_municipal.cantidad_artesanos_activos IS 'Estimación de artesanos activos en la producción evaluada.';
...
-- **********************************************************************
-- Schema: cultura
-- Patrimonio inmaterial, gastronomía tradicional, diversidad lingüística
-- y festividades patronales de Guatemala.
-- **********************************************************************
-- **********************************************************************
...
-- 1. plato_tipico
-- ============================================================
CREATE TABLE IF NOT EXISTS cultura.plato_tipico (
    id              INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    nombre          VARCHAR(150) NOT NULL UNIQUE,
    descripcion     TEXT,
...
CREATE TRIGGER trg_auditoria AFTER INSERT OR DELETE OR UPDATE ON cultura.plato_tipico FOR EACH ROW EXECUTE FUNCTION auditoria.fn_auditar();


-- ============================================================
-- 2. ingrediente_autoctono
-- ============================================================
CREATE TABLE IF NOT EXISTS cultura.ingrediente_autoctono (
    id                  INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
...
COMMENT ON COLUMN cultura.ingrediente_autoctono.descripcion IS 'Descripción de las propiedades o uso del ingrediente.';
COMMENT ON COLUMN cultura.ingrediente_autoctono.origen_prehispanico IS 'Indica si el ingrediente tiene origen prehispánico en la región.';
...
CREATE TRIGGER trg_auditoria AFTER INSERT OR DELETE OR UPDATE ON cultura.ingrediente_autoctono FOR EACH ROW EXECUTE FUNCTION auditoria.fn_auditar();


-- ============================================================
-- 3. plato_ingrediente
-- ============================================================
CREATE TABLE IF NOT EXISTS cultura.plato_ingrediente (
    plato_id       INT NOT NULL REFERENCES cultura.plato_tipico(id) ON DELETE CASCADE,
    ingrediente_id INT NOT NULL REFERENCES cultura.ingrediente_autoctono(id) ON DELETE CASCADE,
    PRIMARY KEY (plato_id, ingrediente_id)
);

COMMENT ON TABLE cultura.plato_ingrediente IS 'Tabla asociativa que relaciona los platos típicos con sus ingredientes característicos.';
COMMENT ON COLUMN cultura.plato_ingrediente.plato_id IS 'Identificador único del plato típico.';
COMMENT ON COLUMN cultura.plato_ingrediente.ingrediente_id IS 'Identificador único del ingrediente.';

CREATE TRIGGER trg_auditoria AFTER INSERT OR DELETE OR UPDATE ON cultura.plato_ingrediente FOR EACH ROW EXECUTE FUNCTION auditoria.fn_auditar();


-- ============================================================
-- 4. plato_geografia
-- ============================================================
CREATE TABLE IF NOT EXISTS cultura.plato_geografia (
    plato_id        INT NOT NULL REFERENCES cultura.plato_tipico(id) ON DELETE CASCADE,
    departamento_id INT NOT NULL REFERENCES geografia.departamento(id) ON DELETE RESTRICT,
    municipio_id    INT REFERENCES geografia.municipio(id) ON DELETE RESTRICT,
    PRIMARY KEY (plato_id, departamento_id)
);

COMMENT ON TABLE cultura.plato_geografia IS 'Relación geográfica de origen o arraigo de los platos típicos (departamento y opcionalmente municipio).';
COMMENT ON COLUMN cultura.plato_geografia.plato_id IS 'Identificador único del plato típico.';
COMMENT ON COLUMN cultura.plato_geografia.departamento_id IS 'Departamento de origen o arraigo del plato.';
COMMENT ON COLUMN cultura.plato_geografia.municipio_id IS 'Municipio específico de origen o arraigo (opcional).';

CREATE TRIGGER trg_auditoria AFTER INSERT OR DELETE OR UPDATE ON cultura.plato_geografia FOR EACH ROW EXECUTE FUNCTION auditoria.fn_auditar();


-- ============================================================
-- 5. idioma
-- ============================================================
CREATE TABLE IF NOT EXISTS cultura.idioma (
    id                  INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    nombre              VARCHAR(100) NOT NULL UNIQUE,
...
COMMENT ON TABLE cultura.idioma IS 'Catálogo de idiomas nacionales hablados en el territorio guatemalteco (mayas, garífuna, xinka y español).';
COMMENT ON COLUMN cultura.idioma.id IS 'Identificador único del idioma.';
COMMENT ON COLUMN cultura.idioma.nombre IS 'Nombre oficial del idioma.';
...
COMMENT ON COLUMN cultura.idioma.estado_vitalidad IS 'Estado de vitalidad del idioma (ej: Vital, En peligro, Crítico).';
COMMENT ON COLUMN cultura.idioma.descripcion IS 'Reseña e información relevante sobre la historia o distribución del idioma.';
COMMENT ON COLUMN cultura.idioma.carga_id IS 'Referencia a la carga de datos mediante la cual se registró el idioma.';

CREATE TRIGGER trg_auditoria AFTER INSERT OR DELETE OR UPDATE ON cultura.idioma FOR EACH ROW EXECUTE FUNCTION auditoria.fn_auditar();


-- ============================================================
-- 6. idioma_municipio
-- ============================================================
CREATE TABLE IF NOT EXISTS cultura.idioma_municipio (
    idioma_id        INT NOT NULL REFERENCES cultura.idioma(id) ON DELETE CASCADE,
    municipio_id     INT NOT NULL REFERENCES geografia.municipio(id) ON DELETE CASCADE,
    es_predominante  BOOLEAN DEFAULT false,
    PRIMARY KEY (idioma_id, municipio_id)
);

COMMENT ON TABLE cultura.idioma_municipio IS 'Tabla asociativa que mapea la distribución territorial y relevancia de los idiomas en los municipios.';
COMMENT ON COLUMN cultura.idioma_municipio.idioma_id IS 'Identificador único del idioma.';
COMMENT ON COLUMN cultura.idioma_municipio.municipio_id IS 'Identificador único del municipio.';
COMMENT ON COLUMN cultura.idioma_municipio.es_predominante IS 'Indica si el idioma es el predominante en el municipio.';

CREATE TRIGGER trg_auditoria AFTER INSERT OR DELETE OR UPDATE ON cultura.idioma_municipio FOR EACH ROW EXECUTE FUNCTION auditoria.fn_auditar();


-- ============================================================
-- 7. evento_cultural
-- ============================================================
CREATE TABLE IF NOT EXISTS cultura.evento_cultural (
    id                    INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    ...
    carga_id              INT REFERENCES meta.carga(id),
    CONSTRAINT evento_cultural_dia_fin_check CHECK (dia_fin >= 1 AND dia_fin <= 31),
    CONSTRAINT evento_cultural_dia_inicio_check CHECK (dia_inicio >= 1 AND dia_inicio <= 31),
    CONSTRAINT evento_cultural_mes_celebracion_check CHECK (mes_celebracion >= 1 AND mes_celebracion <= 12)
);

COMMENT ON TABLE cultura.evento_cultural IS 'Registro de festividades, ferias patronales, festivales y ceremonias tradicionales.';
COMMENT ON COLUMN cultura.evento_cultural.id IS 'Identificador único del evento.';
COMMENT ON COLUMN cultura.evento_cultural.nombre IS 'Nombre oficial del evento o festividad.';
COMMENT ON COLUMN cultura.evento_cultural.tipo_evento IS 'Tipo de festividad (ej: Feria Patronal, Festival Artístico, Ceremonia Espiritual).';
COMMENT ON COLUMN cultura.evento_cultural.mes_celebracion IS 'Mes del año en que se realiza la celebración (1-12).';
COMMENT ON COLUMN cultura.evento_cultural.dia_inicio IS 'Día del mes en que inicia la festividad.';
COMMENT ON COLUMN cultura.evento_cultural.dia_fin IS 'Día del mes en que finaliza la festividad.';
COMMENT ON COLUMN cultura.evento_cultural.descripcion IS 'Descripción del contexto, tradición y actividades del evento.';
COMMENT ON COLUMN cultura.evento_cultural.recomendaciones_viaje IS 'Consejos prácticos para viajeros que desean asistir al evento.';
...
CREATE TRIGGER trg_auditoria AFTER INSERT OR DELETE OR UPDATE ON cultura.evento_cultural FOR EACH ROW EXECUTE FUNCTION auditoria.fn_auditar();


-- ============================================================
-- 8. evento_municipio
-- ============================================================
CREATE TABLE IF NOT EXISTS cultura.evento_municipio (
    evento_id    INT NOT NULL REFERENCES cultura.evento_cultural(id) ON DELETE CASCADE,
    municipio_id INT NOT NULL REFERENCES geografia.municipio(id) ON DELETE CASCADE,
    PRIMARY KEY (evento_id, municipio_id)
);

COMMENT ON TABLE cultura.evento_municipio IS 'Tabla asociativa que mapea la ubicación municipal donde se celebran los eventos culturales.';
COMMENT ON COLUMN cultura.evento_municipio.evento_id IS 'Identificador único del evento.';
COMMENT ON COLUMN cultura.evento_municipio.municipio_id IS 'Identificador único del municipio.';

CREATE TRIGGER trg_auditoria AFTER INSERT OR DELETE OR UPDATE ON cultura.evento_municipio FOR EACH ROW EXECUTE FUNCTION auditoria.fn_auditar();


-- ============================================================
-- Índices
-- ============================================================
CREATE INDEX IF NOT EXISTS ix_evento_municipio_mun ON cultura.evento_municipio(municipio_id);
CREATE INDEX IF NOT EXISTS ix_idioma_municipio_mun ON cultura.idioma_municipio(municipio_id);
CREATE INDEX IF NOT EXISTS ix_plato_geografia_depto_mun ON cultura.plato_geografia(departamento_id, municipio_id);
CREATE INDEX IF NOT EXISTS ix_plato_ingrediente_ingrediente ON cultura.plato_ingrediente(ingrediente_id);
...
-- **********************************************************************
-- Schema: economia
-- Esquema avanzado para el análisis socioeconómico municipal, integrando
-- agricultura, artesanías, cooperativas, materias primas e infraestructura comercial.
-- **********************************************************************
-- **********************************************************************
...
-- 1. materia_prima
-- ============================================================
CREATE TABLE IF NOT EXISTS economia.materia_prima (
    id          INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    nombre      VARCHAR(100) NOT NULL UNIQUE,
    origen      VARCHAR(50),
    descripcion TEXT,
...
COMMENT ON TABLE economia.materia_prima IS 'Materias primas producidas o demandadas en el Altiplano Occidental (ej: Lana, Arcilla, Café en cereza).';
COMMENT ON COLUMN economia.materia_prima.id IS 'Identificador único de la materia prima.';
COMMENT ON COLUMN economia.materia_prima.nombre IS 'Nombre oficial de la materia prima.';
COMMENT ON COLUMN economia.materia_prima.origen IS 'Origen de la materia prima (ej: VEGETAL, ANIMAL, MINERAL).';
COMMENT ON COLUMN economia.materia_prima.descripcion IS 'Descripción de las propiedades o uso comercial de la materia prima.';
COMMENT ON COLUMN economia.materia_prima.carga_id IS 'Referencia a la carga de datos mediante la cual se registró el objeto.';

CREATE TRIGGER trg_auditoria AFTER INSERT OR DELETE OR UPDATE ON economia.materia_prima FOR EACH ROW EXECUTE FUNCTION auditoria.fn_auditar();


-- ============================================================
-- 2. actividad_productiva
-- ============================================================
CREATE TABLE IF NOT EXISTS economia.actividad_productiva (
    id          INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    nombre      VARCHAR(150) NOT NULL UNIQUE,
...
COMMENT ON TABLE economia.actividad_productiva IS 'Catálogo de actividades productivas o cadenas de valor analizadas (ej: Cultivo de Papa, Tejido en Telar).';
COMMENT ON COLUMN economia.actividad_productiva.id IS 'Identificador único de la actividad productiva.';
COMMENT ON COLUMN economia.actividad_productiva.nombre IS 'Nombre oficial de la actividad productiva.';
COMMENT ON COLUMN economia.actividad_productiva.categoria IS 'Categoría económica general (ej: AGRICULTURA, ARTESANIA, PECUARIO).';
COMMENT ON COLUMN economia.actividad_productiva.descripcion IS 'Descripción general de la cadena de valor o proceso productivo.';
COMMENT ON COLUMN economia.actividad_productiva.carga_id IS 'Referencia a la carga de datos.';

CREATE TRIGGER trg_auditoria AFTER INSERT OR DELETE OR UPDATE ON economia.actividad_productiva FOR EACH ROW EXECUTE FUNCTION auditoria.fn_auditar();


-- ============================================================
-- 3. actividad_materia
-- ============================================================
CREATE TABLE IF NOT EXISTS economia.actividad_materia (
    actividad_id     INT NOT NULL REFERENCES economia.actividad_productiva(id) ON DELETE CASCADE,
    materia_id       INT NOT NULL REFERENCES economia.materia_prima(id) ON DELETE CASCADE,
    es_indispensable BOOLEAN DEFAULT true,
    PRIMARY KEY (actividad_id, materia_id)
);

COMMENT ON TABLE economia.actividad_materia IS 'Tabla asociativa que relaciona actividades productivas con las materias primas que producen o consumen.';
COMMENT ON COLUMN economia.actividad_materia.actividad_id IS 'Identificador único de la actividad productiva.';
COMMENT ON COLUMN economia.actividad_materia.materia_id IS 'Identificador único de la materia prima.';
COMMENT ON COLUMN economia.actividad_materia.es_indispensable IS 'Indica si la materia prima es indispensable para el desarrollo de la actividad.';

CREATE TRIGGER trg_auditoria AFTER INSERT OR DELETE OR UPDATE ON economia.actividad_materia FOR EACH ROW EXECUTE FUNCTION auditoria.fn_auditar();


-- ============================================================
-- 4. certificacion
-- ============================================================
CREATE TABLE IF NOT EXISTS economia.certificacion (
    id               INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    nombre           VARCHAR(150) NOT NULL UNIQUE,
    ente_certificador VARCHAR(180),
    descripcion      TEXT,
    carga_id         INT REFERENCES meta.carga(id)
);

COMMENT ON TABLE economia.certificacion IS 'Catálogo de certificaciones de calidad, ambientales o de comercio justo aplicables a la producción local (ej: Orgánico, Fairtrade).';
COMMENT ON COLUMN economia.certificacion.id IS 'Identificador único de la certificación.';
COMMENT ON COLUMN economia.certificacion.nombre IS 'Nombre oficial de la certificación.';
COMMENT ON COLUMN economia.certificacion.ente_certificador IS 'Institución u organismo nacional o internacional responsable de certificar.';
COMMENT ON COLUMN economia.certificacion.descripcion IS 'Descripción de los criterios necesarios para obtener la certificación.';
COMMENT ON COLUMN economia.certificacion.carga_id IS 'Referencia a la carga de datos.';

CREATE TRIGGER trg_auditoria AFTER INSERT OR DELETE OR UPDATE ON economia.certificacion FOR EACH ROW EXECUTE FUNCTION auditoria.fn_auditar();


-- ============================================================
-- 5. cooperativa_asociacion
-- ============================================================
CREATE TABLE IF NOT EXISTS economia.cooperativa_asociacion (
    id           INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    nombre       VARCHAR(220) NOT NULL UNIQUE,
    siglas       VARCHAR(50),
...
COMMENT ON TABLE economia.cooperativa_asociacion IS 'Registro de cooperativas, asociaciones de productores y comités organizados de economía solidaria.';
COMMENT ON COLUMN economia.cooperativa_asociacion.id IS 'Identificador único del colectivo organizado.';
COMMENT ON COLUMN economia.cooperativa_asociacion.nombre IS 'Nombre oficial o razón social de la cooperativa/asociación.';
COMMENT ON COLUMN economia.cooperativa_asociacion.siglas IS 'Siglas o acrónimo representativo de la organización.';
COMMENT ON COLUMN economia.cooperativa_asociacion.municipio_id IS 'Municipio oficial verificado de la sede principal.';
COMMENT ON COLUMN economia.cooperativa_asociacion.cobertura IS 'Nivel geográfico de cobertura (ej: LOCAL, DEPARTAMENTAL, REGIONAL).';
COMMENT ON COLUMN economia.cooperativa_asociacion.descripcion IS 'Descripción del objeto social, servicios y trayectoria de la organización.';
COMMENT ON COLUMN economia.cooperativa_asociacion.carga_id IS 'Referencia a la carga de datos.';

CREATE TRIGGER trg_auditoria AFTER INSERT OR DELETE OR UPDATE ON economia.cooperativa_asociacion FOR EACH ROW EXECUTE FUNCTION auditoria.fn_auditar();


-- ============================================================
-- 6. produccion_municipal
-- ============================================================
CREATE TABLE IF NOT EXISTS economia.produccion_municipal (
    municipio_id               INT NOT NULL REFERENCES geografia.municipio(id) ON DELETE RESTRICT,
    actividad_id               INT NOT NULL REFERENCES economia.actividad_productiva(id) ON DELETE CASCADE,
    cooperativa_id             INT REFERENCES economia.cooperativa_asociacion(id) ON DELETE SET NULL,
    es_principal               BOOLEAN DEFAULT false,
    volumen_estimado           VARCHAR(100),
    cantidad_productores_est   INT,
    empleo_generado_est        INT,
    ciclo_cosecha_meses        INT,
    destino_principal          VARCHAR(150),
    carga_id                   INT REFERENCES meta.carga(id),
    PRIMARY KEY (municipio_id, actividad_id)
);

COMMENT ON TABLE economia.produccion_municipal IS 'Estimaciones anuales y caracterización económica de las actividades productivas a nivel municipal.';
COMMENT ON COLUMN economia.produccion_municipal.municipio_id IS 'Municipio oficial de estudio.';
COMMENT ON COLUMN economia.produccion_municipal.actividad_id IS 'Actividad productiva evaluada.';
COMMENT ON COLUMN economia.produccion_municipal.cooperativa_id IS 'Cooperativa u organización líder de la actividad en el municipio (si aplica).';
COMMENT ON COLUMN economia.produccion_municipal.es_principal IS 'Indica si es una de las actividades económicas principales del municipio.';
COMMENT ON COLUMN economia.produccion_municipal.volumen_estimado IS 'Volumen de producción estimado (ej: "15,000 quintales", "5,000 yardas").';
COMMENT ON COLUMN economia.produccion_municipal.cantidad_productores_est IS 'Estimación de cantidad de productores dedicados a la actividad.';
COMMENT ON COLUMN economia.produccion_municipal.empleo_generado_est IS 'Estimación de empleos directos e indirectos generados en la cadena.';
COMMENT ON COLUMN economia.produccion_municipal.ciclo_cosecha_meses IS 'Meses de duración del ciclo de siembra/cosecha o producción anual.';
COMMENT ON COLUMN economia.produccion_municipal.destino_principal IS 'Destino principal de la producción (ej: LOCAL, NACIONAL, EXPORTACION).';
COMMENT ON COLUMN economia.produccion_municipal.carga_id IS 'Referencia a la carga de datos.';

CREATE TRIGGER trg_auditoria AFTER INSERT OR DELETE OR UPDATE ON economia.produccion_municipal FOR EACH ROW EXECUTE FUNCTION auditoria.fn_auditar();


-- ============================================================
-- 7. produccion_certificada
-- ============================================================
CREATE TABLE IF NOT EXISTS economia.produccion_certificada (
...
COMMENT ON TABLE economia.produccion_certificada IS 'Registro de qué porción de la producción de un municipio cuenta con certificaciones internacionales.';
COMMENT ON COLUMN economia.produccion_certificada.municipio_id IS 'Municipio oficial de estudio.';
COMMENT ON COLUMN economia.produccion_certificada.actividad_id IS 'Actividad productiva certificada.';
COMMENT ON COLUMN economia.produccion_certificada.certificacion_id IS 'Certificación obtenida.';
COMMENT ON COLUMN economia.produccion_certificada.porcentaje_produccion IS 'Porcentaje estimado de la producción que cumple con la certificación.';
COMMENT ON COLUMN economia.produccion_certificada.fecha_auditoria IS 'Fecha de la última auditoría o renovación de la certificación.';
COMMENT ON COLUMN economia.produccion_certificada.carga_id IS 'Referencia a la carga de datos.';

CREATE TRIGGER trg_auditoria AFTER INSERT OR DELETE OR UPDATE ON economia.produccion_certificada FOR EACH ROW EXECUTE FUNCTION auditoria.fn_auditar();


-- ============================================================
-- 8. centro_acopio_procesamiento
-- ============================================================
CREATE TABLE IF NOT EXISTS economia.centro_acopio_procesamiento (
    id                 INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    nombre             VARCHAR(220) NOT NULL,
    tipo               VARCHAR(80),
    municipio_id       INT NOT NULL REFERENCES geografia.municipio(id) ON DELETE RESTRICT,
    capacidad_estimada VARCHAR(150),
    carga_id           INT REFERENCES meta.carga(id),
    CONSTRAINT cap_nombre_municipio_id_key UNIQUE (nombre, municipio_id)
);

COMMENT ON TABLE economia.centro_acopio_procesamiento IS 'Infraestructura productiva: centros de acopio, plantas de procesamiento o descascaradoras locales.';
COMMENT ON COLUMN economia.centro_acopio_procesamiento.id IS 'Identificador único del centro de acopio/procesamiento.';
COMMENT ON COLUMN economia.centro_acopio_procesamiento.nombre IS 'Nombre descriptivo del centro o planta.';
COMMENT ON COLUMN economia.centro_acopio_procesamiento.tipo IS 'Tipo de infraestructura (ej: BENEFICIO DE CAFE, CENTRO DE ACOPIO DE PAPA).';
COMMENT ON COLUMN economia.centro_acopio_procesamiento.municipio_id IS 'Municipio de ubicación física.';
COMMENT ON COLUMN economia.centro_acopio_procesamiento.capacidad_estimada IS 'Capacidad operativa o de almacenamiento estimada.';
COMMENT ON COLUMN economia.centro_acopio_procesamiento.carga_id IS 'Referencia a la carga de datos.';

CREATE TRIGGER trg_auditoria AFTER INSERT OR DELETE OR UPDATE ON economia.centro_acopio_procesamiento FOR EACH ROW EXECUTE FUNCTION auditoria.fn_auditar();


-- ============================================================
-- 9. mercado_tradicional
-- ============================================================
CREATE TABLE IF NOT EXISTS economia.mercado_tradicional (
    id                      INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    nombre                  VARCHAR(220) NOT NULL UNIQUE,
    municipio_id            INT NOT NULL REFERENCES geografia.municipio(id) ON DELETE RESTRICT,
    dias_plaza              VARCHAR(150),
    tipo_mercado            VARCHAR(80),
...
COMMENT ON TABLE economia.mercado_tradicional IS 'Mercados municipales y días de plaza tradicionales del Altiplano Occidental.';
COMMENT ON COLUMN economia.mercado_tradicional.id IS 'Identificador único del mercado.';
COMMENT ON COLUMN economia.mercado_tradicional.nombre IS 'Nombre descriptivo del mercado tradicional (ej: Mercado Central de San Francisco El Alto).';
COMMENT ON COLUMN economia.mercado_tradicional.municipio_id IS 'Municipio oficial verificado de ubicación.';
COMMENT ON COLUMN economia.mercado_tradicional.dias_plaza IS 'Días del año o días de la semana específicos de plaza (ej: "Jueves y Domingo").';
COMMENT ON COLUMN economia.mercado_tradicional.tipo_mercado IS 'Clasificación del mercado (ej: MAYORISTA, MINORISTA, ARTESANAL).';
COMMENT ON COLUMN economia.mercado_tradicional.cantidad_vendedores_est IS 'Estimación de cantidad de vendedores en días de máxima plaza.';
COMMENT ON COLUMN economia.mercado_tradicional.latitud IS 'Latitud para georreferenciación del mercado.';
COMMENT ON COLUMN economia.mercado_tradicional.longitud IS 'Longitud para georreferenciación del mercado.';
COMMENT ON COLUMN economia.mercado_tradicional.descripcion IS 'Descripción del flujo, productos dominantes e historia del mercado.';
COMMENT ON COLUMN economia.mercado_tradicional.carga_id IS 'Referencia a la carga de datos.';

CREATE TRIGGER trg_auditoria AFTER INSERT OR DELETE OR UPDATE ON economia.mercado_tradicional FOR EACH ROW EXECUTE FUNCTION auditoria.fn_auditar();


-- ============================================================
-- 10. ruta_comercio
-- ============================================================
CREATE TABLE IF NOT EXISTS economia.ruta_comercio (
    id                    INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    origen_municipio_id   INT NOT NULL REFERENCES geografia.municipio(id) ON DELETE RESTRICT,
    destino_municipio_id  INT NOT NULL REFERENCES geografia.municipio(id) ON DELETE RESTRICT,
    puerto_salida         VARCHAR(150),
...
COMMENT ON TABLE economia.ruta_comercio IS 'Rutas de transporte y distribución física de productos agrícolas y artesanales hacia mercados nacionales o puertos.';
COMMENT ON COLUMN economia.ruta_comercio.id IS 'Identificador único del tramo o ruta.';
COMMENT ON COLUMN economia.ruta_comercio.origen_municipio_id IS 'Municipio oficial de origen de la producción.';
COMMENT ON COLUMN economia.ruta_comercio.destino_municipio_id IS 'Municipio oficial de destino final o centro de distribución principal.';
COMMENT ON COLUMN economia.ruta_comercio.puerto_salida IS 'Puerto marítimo, frontera terrestre o aeropuerto de salida internacional (si aplica).';
COMMENT ON COLUMN economia.ruta_comercio.medio_transporte IS 'Medio de transporte principal utilizado (ej: TERRESTRE - CAMION, TERRESTRE - PICKUP).';
COMMENT ON COLUMN economia.ruta_comercio.distancia_km IS 'Distancia estimada en kilómetros.';
COMMENT ON COLUMN economia.ruta_comercio.tiempo_estimado_horas IS 'Tiempo estimado de viaje en horas decimales.';
COMMENT ON COLUMN economia.ruta_comercio.producto_principal IS 'Producto principal distribuido en la ruta.';
COMMENT ON COLUMN economia.ruta_comercio.carga_id IS 'Referencia a la carga de datos.';

CREATE TRIGGER trg_auditoria AFTER INSERT OR DELETE OR UPDATE ON economia.ruta_comercio FOR EACH ROW EXECUTE FUNCTION auditoria.fn_auditar();


-- ============================================================
-- 11. servicio_financiero
-- ============================================================
CREATE TABLE IF NOT EXISTS economia.servicio_financiero (
    id                    INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    ...
    monto_maximo_quetzales NUMERIC(12,2),
    requisito_principal   TEXT,
    carga_id              INT REFERENCES meta.carga(id),
    CONSTRAINT sf_cooperativa_id_tipo_servicio_key UNIQUE (cooperativa_id, tipo_servicio)
);

COMMENT ON TABLE economia.servicio_financiero IS 'Servicios de microcrédito, ahorro y seguros de desarrollo ofertados por cooperativas de ahorro y crédito.';
COMMENT ON COLUMN economia.servicio_financiero.id IS 'Identificador único del servicio financiero.';
COMMENT ON COLUMN economia.servicio_financiero.cooperativa_id IS 'Cooperativa que oferta el servicio.';
COMMENT ON COLUMN economia.servicio_financiero.tipo_servicio IS 'Tipo de servicio (ej: CREDITO AGRICOLA, MICROCREDITO MUJERES, SEGURO AGRÍCOLA).';
COMMENT ON COLUMN economia.servicio_financiero.tasa_interes_anual IS 'Tasa de interés anual activa/pasiva aproximada.';
COMMENT ON COLUMN economia.servicio_financiero.monto_maximo_quetzales IS 'Monto máximo financiado en Quetzales.';
COMMENT ON COLUMN economia.servicio_financiero.requisito_principal IS 'Requisito principal o colateral exigido.';
COMMENT ON COLUMN economia.servicio_financiero.carga_id IS 'Referencia a la carga de datos.';

CREATE TRIGGER trg_auditoria AFTER INSERT OR DELETE OR UPDATE ON economia.servicio_financiero FOR EACH ROW EXECUTE FUNCTION auditoria.fn_auditar();


-- ============================================================
-- Índices
-- ============================================================
CREATE INDEX IF NOT EXISTS ix_actividad_materia_materia ON economia.actividad_materia(materia_id);
CREATE INDEX IF NOT EXISTS ix_centro_acopio_mun ON economia.centro_acopio_procesamiento(municipio_id);
CREATE INDEX IF NOT EXISTS ix_cooperativa_mun ON economia.cooperativa_asociacion(municipio_id);
CREATE INDEX IF NOT EXISTS ix_mercado_mun ON economia.mercado_tradicional(municipio_id);
CREATE INDEX IF NOT EXISTS ix_produccion_municipal_actividad ON economia.produccion_municipal(actividad_id);
CREATE INDEX IF NOT EXISTS ix_produccion_municipal_cooperativa ON economia.produccion_municipal(cooperativa_id);
...
-- **********************************************************************
-- Schema: transporte
-- Módulo de Logística, Transporte y Conectividad Vial, enfocado en facilitar
-- información de transporte para turistas e investigadores en el Altiplano Occidental.
-- **********************************************************************
-- **********************************************************************
...
CREATE TABLE IF NOT EXISTS transporte.tipo_transporte (
    id_tipo_transporte INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    codigo             VARCHAR(50) NOT NULL UNIQUE,
    nombre             VARCHAR(100) NOT NULL UNIQUE,
    descripcion        TEXT,
    carga_id           INT REFERENCES meta.carga(id)
...
COMMENT ON TABLE transporte.tipo_transporte IS 'Tipos de transporte disponibles (ej: Microbús, Tuc-tuc, Autobús extraurbano, Lancha).';
COMMENT ON COLUMN transporte.tipo_transporte.id_tipo_transporte IS 'Identificador único del tipo de transporte.';
COMMENT ON COLUMN transporte.tipo_transporte.codigo IS 'Código único de identificación.';
COMMENT ON COLUMN transporte.tipo_transporte.nombre IS 'Nombre descriptivo del tipo de transporte.';
COMMENT ON COLUMN transporte.tipo_transporte.descripcion IS 'Descripción del tipo de vehículo y su uso.';
COMMENT ON COLUMN transporte.tipo_transporte.carga_id IS 'Referencia a la carga de datos.';

CREATE TRIGGER trg_auditoria AFTER INSERT OR DELETE OR UPDATE ON transporte.tipo_transporte FOR EACH ROW EXECUTE FUNCTION auditoria.fn_auditar();
...
CREATE TABLE IF NOT EXISTS transporte.tipo_via (
    id_tipo_via INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    codigo      VARCHAR(50) NOT NULL UNIQUE,
    nombre      VARCHAR(100) NOT NULL UNIQUE,
    descripcion TEXT,
    carga_id    INT REFERENCES meta.carga(id)
...
COMMENT ON TABLE transporte.tipo_via IS 'Clasificación de vías de comunicación (ej: Carretera Asfaltada, Terracería, Vía Lacustre).';
COMMENT ON COLUMN transporte.tipo_via.id_tipo_via IS 'Identificador único del tipo de vía.';
COMMENT ON COLUMN transporte.tipo_via.codigo IS 'Código único de identificación.';
COMMENT ON COLUMN transporte.tipo_via.nombre IS 'Nombre del tipo de vía.';
COMMENT ON COLUMN transporte.tipo_via.descripcion IS 'Descripción de la infraestructura de la vía.';
COMMENT ON COLUMN transporte.tipo_via.carga_id IS 'Referencia a la carga de datos.';

CREATE TRIGGER trg_auditoria AFTER INSERT OR DELETE OR UPDATE ON transporte.tipo_via FOR EACH ROW EXECUTE FUNCTION auditoria.fn_auditar();


-- ============================================================
-- 3. conexion_municipal
-- ============================================================
CREATE TABLE IF NOT EXISTS transporte.conexion_municipal (
    id_conexion             INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    municipio_id            INT NOT NULL REFERENCES geografia.municipio(id),
    tipo_vehiculo           VARCHAR(50) NOT NULL,
    distancia_cabecera_km   NUMERIC(6,2),
    tiempo_cabecera_minutos INT,
    distancia_capital_km    NUMERIC(6,2),
    tiempo_capital_minutos  INT,
    carga_id                INT REFERENCES meta.carga(id),
    CONSTRAINT uq_conexion_muni_vehiculo UNIQUE (municipio_id, tipo_vehiculo)
);

COMMENT ON TABLE transporte.conexion_municipal IS 'Tiempos y distancias promedio desde el municipio hacia la cabecera departamental y la Ciudad de Guatemala.';
COMMENT ON COLUMN transporte.conexion_municipal.id_conexion IS 'Identificador único de la conexión.';
COMMENT ON COLUMN transporte.conexion_municipal.municipio_id IS 'Municipio de origen.';
COMMENT ON COLUMN transporte.conexion_municipal.tipo_vehiculo IS 'Tipo de vehículo utilizado en la medición de la ruta.';
COMMENT ON COLUMN transporte.conexion_municipal.distancia_cabecera_km IS 'Distancia hacia la cabecera departamental en kilómetros.';
COMMENT ON COLUMN transporte.conexion_municipal.tiempo_cabecera_minutos IS 'Tiempo estimado de viaje hacia la cabecera departamental.';
COMMENT ON COLUMN transporte.conexion_municipal.distancia_capital_km IS 'Distancia hacia la Ciudad de Guatemala en kilómetros.';
COMMENT ON COLUMN transporte.conexion_municipal.tiempo_capital_minutos IS 'Tiempo estimado de viaje hacia la Ciudad de Guatemala.';
COMMENT ON COLUMN transporte.conexion_municipal.carga_id IS 'Referencia a la carga de datos.';

CREATE TRIGGER trg_auditoria AFTER INSERT OR DELETE OR UPDATE ON transporte.conexion_municipal FOR EACH ROW EXECUTE FUNCTION auditoria.fn_auditar();


-- ============================================================
-- 4. proveedor_transporte
-- ============================================================
...
COMMENT ON TABLE transporte.proveedor_transporte IS 'Empresas, cooperativas o transportistas individuales que prestan servicios de transporte.';
...
COMMENT ON COLUMN transporte.proveedor_transporte.nombre IS 'Nombre oficial o comercial del proveedor.';
COMMENT ON COLUMN transporte.proveedor_transporte.representante IS 'Representante legal o encargado del proveedor.';
COMMENT ON COLUMN transporte.proveedor_transporte.contacto IS 'Datos de contacto del proveedor.';
COMMENT ON COLUMN transporte.proveedor_transporte.departamento_id IS 'Departamento de la sede principal.';
COMMENT ON COLUMN transporte.proveedor_transporte.municipio_id IS 'Municipio de la sede principal.';
COMMENT ON COLUMN transporte.proveedor_transporte.carga_id IS 'Referencia a la carga de datos.';

CREATE TRIGGER trg_auditoria AFTER INSERT OR DELETE OR UPDATE ON transporte.proveedor_transporte FOR EACH ROW EXECUTE FUNCTION auditoria.fn_auditar();


-- ============================================================
-- 5. destino_transporte
-- ============================================================
CREATE TABLE IF NOT EXISTS transporte.destino_transporte (
    id_destino_transporte   INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    destino_id              INT NOT NULL REFERENCES turismo.destino_turistico(id_destino) ON DELETE CASCADE,
    parada_cercana          VARCHAR(250) NOT NULL,
    tipo_transporte_id      INT NOT NULL REFERENCES transporte.tipo_transporte(id_tipo_transporte),
    distancia_parada_km     NUMERIC(5,2),
    costo_traslado_local_q  NUMERIC(10,2),
    tiempo_traslado_minutos INT,
    carga_id                INT REFERENCES meta.carga(id),
    CONSTRAINT uq_destino_transporte_parada UNIQUE (destino_id, parada_cercana, tipo_transporte_id)
);

COMMENT ON TABLE transporte.destino_transporte IS 'Opciones de conectividad y transporte específicas para llegar a los destinos turísticos.';
COMMENT ON COLUMN transporte.destino_transporte.id_destino_transporte IS 'Identificador único.';
COMMENT ON COLUMN transporte.destino_transporte.destino_id IS 'Destino turístico de referencia.';
COMMENT ON COLUMN transporte.destino_transporte.parada_cercana IS 'Nombre o ubicación de la parada o estación más cercana al destino.';
COMMENT ON COLUMN transporte.destino_transporte.tipo_transporte_id IS 'Tipo de transporte que arriba a dicha parada.';
COMMENT ON COLUMN transporte.destino_transporte.distancia_parada_km IS 'Distancia desde la parada hasta el punto exacto del destino.';
COMMENT ON COLUMN transporte.destino_transporte.costo_traslado_local_q IS 'Costo estimado de traslado local en Quetzales.';
COMMENT ON COLUMN transporte.destino_transporte.tiempo_traslado_minutos IS 'Tiempo estimado de traslado en minutos.';
COMMENT ON COLUMN transporte.destino_transporte.carga_id IS 'Referencia a la carga de datos.';

CREATE TRIGGER trg_auditoria AFTER INSERT OR DELETE OR UPDATE ON transporte.destino_transporte FOR EACH ROW EXECUTE FUNCTION auditoria.fn_auditar();
...
CREATE TABLE IF NOT EXISTS transporte.ruta_transporte (
    id_ruta_transporte   INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    codigo               VARCHAR(50) NOT NULL UNIQUE,
...
COMMENT ON TABLE transporte.ruta_transporte IS 'Rutas de transporte intermunicipales autorizadas.';
COMMENT ON COLUMN transporte.ruta_transporte.id_ruta_transporte IS 'Identificador único de la ruta.';
COMMENT ON COLUMN transporte.ruta_transporte.codigo IS 'Código estable de la ruta.';
COMMENT ON COLUMN transporte.ruta_transporte.origen_municipio_id IS 'Municipio de origen.';
COMMENT ON COLUMN transporte.ruta_transporte.destino_municipio_id IS 'Municipio de destino.';
COMMENT ON COLUMN transporte.ruta_transporte.tipo_transporte_id IS 'Tipo de transporte utilizado en la ruta.';
COMMENT ON COLUMN transporte.ruta_transporte.carga_id IS 'Referencia a la carga de datos.';

CREATE TRIGGER trg_auditoria AFTER INSERT OR DELETE OR UPDATE ON transporte.ruta_transporte FOR EACH ROW EXECUTE FUNCTION auditoria.fn_auditar();


-- ============================================================
-- 7. itinerario_transporte
-- ============================================================
CREATE TABLE IF NOT EXISTS transporte.itinerario_transporte (
    id_itinerario             INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
...
COMMENT ON TABLE transporte.itinerario_transporte IS 'Horarios de salida, tarifas y duración estimada del viaje de los proveedores de transporte.';
COMMENT ON COLUMN transporte.itinerario_transporte.id_itinerario IS 'Identificador único.';
COMMENT ON COLUMN transporte.itinerario_transporte.ruta_transporte_id IS 'Ruta de transporte asociada.';
COMMENT ON COLUMN transporte.itinerario_transporte.proveedor_id IS 'Proveedor que presta el servicio.';
COMMENT ON COLUMN transporte.itinerario_transporte.hora_salida IS 'Hora programada de salida del servicio.';
COMMENT ON COLUMN transporte.itinerario_transporte.frecuencia IS 'Días o intervalos en que se presta el servicio (ej: Lunes a Viernes cada 30 min).';
COMMENT ON COLUMN transporte.itinerario_transporte.tarifa_q IS 'Tarifa oficial en Quetzales.';
COMMENT ON COLUMN transporte.itinerario_transporte.duracion_estimada_minutos IS 'Tiempo estimado de viaje en minutos.';
COMMENT ON COLUMN transporte.itinerario_transporte.carga_id IS 'Referencia a la carga de datos.';

CREATE TRIGGER trg_auditoria AFTER INSERT OR DELETE OR UPDATE ON transporte.itinerario_transporte FOR EACH ROW EXECUTE FUNCTION auditoria.fn_auditar();
...
CREATE TABLE IF NOT EXISTS transporte.tramo_vial (
    ...
    estado_transitabilidad   VARCHAR(50),
    carga_id                 INT REFERENCES meta.carga(id)
);

COMMENT ON TABLE transporte.tramo_vial IS 'Tramos viales específicos entre municipios para medir el estado de la conectividad vial.';
COMMENT ON COLUMN transporte.tramo_vial.id_tramo IS 'Identificador único del tramo vial.';
COMMENT ON COLUMN transporte.tramo_vial.codigo IS 'Código estable de la vía.';
COMMENT ON COLUMN transporte.tramo_vial.nombre IS 'Nombre oficial o descriptivo de la carretera o vía.';
COMMENT ON COLUMN transporte.tramo_vial.origen_municipio_id IS 'Municipio de inicio del tramo.';
COMMENT ON COLUMN transporte.tramo_vial.destino_municipio_id IS 'Municipio de fin del tramo.';
COMMENT ON COLUMN transporte.tramo_vial.tipo_via_id IS 'Clasificación física de la vía.';
COMMENT ON COLUMN transporte.tramo_vial.distancia_km IS 'Distancia real del tramo en kilómetros.';
COMMENT ON COLUMN transporte.tramo_vial.tiempo_promedio_minutos IS 'Tiempo promedio de viaje en condiciones normales.';
COMMENT ON COLUMN transporte.tramo_vial.estado_transitabilidad IS 'Calidad o estado de la vía (ej: BUENO, REGULAR, MALO, INTRANSITABLE).';
COMMENT ON COLUMN transporte.tramo_vial.carga_id IS 'Referencia a la carga de datos.';

CREATE TRIGGER trg_auditoria AFTER INSERT OR DELETE OR UPDATE ON transporte.tramo_vial FOR EACH ROW EXECUTE FUNCTION auditoria.fn_auditar();
...
COMMENT ON TABLE transporte.incidencia_vial IS 'Eventos activos e históricos que afectan la conectividad de los tramos (ej: Derrumbes, Bloqueos, Reparación).';
COMMENT ON COLUMN transporte.incidencia_vial.id_incidencia IS 'Identificador único de la incidencia.';
COMMENT ON COLUMN transporte.incidencia_vial.codigo IS 'Código único de reporte.';
COMMENT ON COLUMN transporte.incidencia_vial.tramo_id IS 'Tramo vial afectado.';
COMMENT ON COLUMN transporte.incidencia_vial.tipo_incidencia IS 'Tipo de incidencia (ej: DERRUMBE, ACCIDENTE, BLOQUEO, REPARACION).';
COMMENT ON COLUMN transporte.incidencia_vial.descripcion IS 'Detalle o reporte de la incidencia.';
COMMENT ON COLUMN transporte.incidencia_vial.fecha_reporte IS 'Fecha y hora en la que se registró el reporte.';
COMMENT ON COLUMN transporte.incidencia_vial.activo IS 'Indica si la incidencia sigue afectando la transitabilidad en la actualidad.';
COMMENT ON COLUMN transporte.incidencia_vial.carga_id IS 'Referencia a la carga de datos.';

CREATE TRIGGER trg_auditoria AFTER INSERT OR DELETE OR UPDATE ON transporte.incidencia_vial FOR EACH ROW EXECUTE FUNCTION auditoria.fn_auditar();


-- ============================================================
-- Índices
-- ============================================================
CREATE INDEX IF NOT EXISTS ix_incidencia_tramo ON transporte.incidencia_vial(tramo_id);
